...
MOTOR_REAL_INFO can1motorRealInfo[7] = {0};//底盘4个3508电机m = 0 1 2 3; 云台2个6020电机m = 4 5（6020驱动器id应当设置为1 2）
MOTOR_REAL_INFO can2motorRealInfo[7] = {0};//履带2个3508电机m = 0 1; 夹爪翻转1个3508电机m = 2; 凸轮2个3508电机m = 3 4; 炮塔云台1个3508电机m = 5
...
void Motor_Control(void)
{
    for(int i = 0; i < 7; i++)
    {
        ...
        switch (can1motorRealInfo[i].Motor_Mode)
        {
            ...
            {
                if (can1motorRealInfo[i].once_flag == 1){//单圈模式位置环
                    pid_calc(&can1MOTOR_PID_POS[i],can1motorRealInfo[i].TARGET_POS, can1motorRealInfo[i].ANGLE * 360 / 8192);//位置环
        ...
        switch (can2motorRealInfo[i].Motor_Mode)
        {
            ...
            case POSITION_CONTROL_MODE://位置模式
            {
                if (can2motorRealInfo[i].once_flag == 1){//单圈模式位置环
                    pid_calc(&can2MOTOR_PID_POS[i],can2motorRealInfo[i].TARGET_POS, can2motorRealInfo[i].ANGLE * 360.0f / 8192);//位置环
